...
export default async function fetchProjects(uuid: string) : Promise<Project[]> {
    ...
    for (const project_item of userProjects) {
        const projectLogStatics = await projectController.getProjectLogStatistics(project_item.project_id);
        const project: Project = { 
            ...
            visits: projectLogStatics?.totalVisits || "NaN",
            users: projectLogStatics?.uniqueVisitors || "NaN"
        }
